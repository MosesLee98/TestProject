...
public class Index {
    ...
    public void confirmInformation() {
		...
		for(String info : databank.contactInfo) {
			Assert.assertTrue(bodyText.contains(databank.loc@@ation));
		}

        Assert.assertTrue(bodyText.contains(databank.location));
    }

    @Test
    public void confirmContactInfo() {
		String bodyText = driver.findElement(By.tagName("body")).getText();
		Assert.assertTrue(bodyText.contains(databank.phone));
    }
